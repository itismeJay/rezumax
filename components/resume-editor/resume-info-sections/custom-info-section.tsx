// app/edit/[resumeId]/custom-info-section.tsx
"use client";
...
export function CustomInfoSection({
  ...
  const [entries, setEntries] = useState<CustomEntry[]>(customInfo);
  const [collapsed, setCollapsed] = useState(false);
  const [visible, setVisible] = useState(externalVisible ?? true);
  const [isRenamingSection, setIsRenamingSection] = useState(false);

  const sectionName = externalSectionName || "Custom Section";
        ...
        <CardContent className="px-5 pb-5 space-y-6">
            ...
            >
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="e.g., Award Name, Project Title, Activity"
                  className="h-10"
                />
              </div>
              ...
              <div className="space-y-2">
                <Label className="text-xs text-muted-foreground font-medium">
                  Subtitle / Organization (Optional)
                </Label>
                <Input
                  value={entry.subtitle}
                  onChange={(e) =>
                    handleFieldChange(entry.id, "subtitle", e.target.value)
                  }
                  placeholder="e.g., Organization Name, Institution"
                  className="h-10"
                />
              </div>
              ...
              <div className="space-y-3">
                ...
                {entry.description.map((desc, descIndex) => (
                  <div key={descIndex} className="flex gap-2 items-start">
                    ...
                    <Textarea
                      placeholder="Add details, achievements, or context..."
          ...
          <div className="p-3 rounded-md bg-muted/50 border border-border">
            <p className="text-xs text-muted-foreground">
              💡 <span className="font-medium">Flexible section:</span> Use this
              for any information not covered by other sections - awards,
              patents, speaking engagements, media coverage, etc.
            </p>
          </div>
        </CardContent>
      )}
